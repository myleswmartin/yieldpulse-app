...
export default function SignInPage() {
  ...
  useEffect(() => {
    console.log("SignInPage: auth state", { authLoading, userId: user?.id, from });
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      // Do NOT navigate here. Let the effect above run after user state updates.
    } catch (err: any) {
      ...
      <div className="w-full max-w-md">
        <div className="text-center mb-12">
          <Link to="/" className="inline-flex items-center space-x-3 mb-10 group">
            ...
            <span className="text-2xl font-semibold text-primary tracking-tight">YieldPulse</span>
          </Link>
          <h1 className="text-3xl font-bold text-foreground mb-3 tracking-tight">Welcome Back</h1>
          <p className="text-neutral-600">Sign in to access your property analyses</p>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-border p-8">
          ...
          <div className="mt-4 text-center">
            <Link to="/" className="inline-flex items-center space-x-2 text-sm text-neutral-500 hover:text-neutral-700 transition-colors">
        ...
        <div className="mt-8 text-center">
          <p className="text-sm text-neutral-500">Trusted by property investors across the UAE</p>
        </div>
      </div>
    </div>
  );
